...
const newTaskAdd = () => {
  ...
  b_d.children[2].innerHTML = 
    `${b_d.children[2].innerHTML}` +`<div id="item_${arr2[arr2.length - 1]}" class="item"> <button style="background-color:skyblue;border-radius:20px;margin:10px;padding:10px;" id="markdone_${arr2[arr2.length - 1]}"onclick="markdone(event)">MarkDone</button ><span>${t}</span ></div>`;
...
const open_list = (event) => {
    ...
    document.getElementById('plus-icon').innerHTML=`${back.innerHTML}  ${document.getElementById('plus-icon').innerHTML}`
